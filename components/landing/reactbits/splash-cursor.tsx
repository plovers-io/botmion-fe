...
export function SplashCursor({
  ...
  useEffect(() => {
    ...
    const isWebGL2 = glCtx instanceof WebGL2RenderingContext;
    ...
    if (isWebGL2) {
      ...
      supportLinearFiltering = glCtx.getExtension(
    ...
    function createProgram(vs: WebGLShader | null, fs: WebGLShader | null) {
      ...
      if (vs) glCtx.attachShader(program, vs);
      if (fs) glCtx.attachShader(program, fs);
    ...
    class Material {
      ...
      uniforms: Record<string, WebGLUniformLocation | null>;
      constructor(vs: WebGLShader | null, fsSource: string) {
    ...
    function drawDisplay(target: FBO | null) {
      const width = target == null ? glCtx.drawingBufferWidth : target.width;
      const height =
      ...
      glCtx.uniform1i(
        (displayMaterial.uniforms as any).uTexture,
        dye.read.attach(0)
      );
      blit(target);
    }
